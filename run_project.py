...
def main():
    ...
    print("\n[Step 2/4] Starting Ingress REST API Server on http://localhost:8000/...")
    producer_proc = subprocess.Popen([sys.executable, "producer/app.py"])
    time.sleep(1.5)

    # 3. Launch Stream Generator
    print("\n[Step 3/4] Starting Continuous Telemetry Generator at 3 TPS...")
    generator_proc = subprocess.Popen([sys.executable, "generator/generator.py", "--tps", "3"])
    time.sleep(1.5)

    # 4. Launch Web Dashboard
    print("\n[Step 4/4] Starting Web Dashboard Server on http://localhost:8050/...")
