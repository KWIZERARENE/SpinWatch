...
def build_machine_catalog(size=1000):
    """Create a 1000-machine fleet with unique random machine IDs and branch spread."""
    machine_numbers = random.sample(range(1, 1001), size)
    return [
        {
            "machine_id": f"WM_{machine_no:04d}",
...
def generate_telemetry():
    ...
    parser.add_argument("--target-url", type=str, default="http://127.0.0.1:8000/api/readings/", help="Target REST API URL")
    ...
    while True:
        ...
        # Generate realistic, non-sequential variation across the 1,000-machine fleet
        base_temp = random.gauss(58.0, 19.0)
        if random.random() < 0.25:
            base_temp += random.uniform(15.0, 32.0)
        current_temp = round(max(24.0, min(110.0, base_temp)), 2)

        vibration_hz = round(random.gauss(48.0, 24.0), 1)
        if vibration_hz < 12.0:
            vibration_hz = 12.0
        if vibration_hz > 120.0:
            vibration_hz = 120.0

        power_kw = round(random.uniform(1.5, 8.5), 2)
        water_pressure_bar = round(random.uniform(1.2, 5.0), 2)
        error_code = random.choice(ERROR_CODES) if current_temp > 70.0 or vibration_hz > 90.0 else "NORMAL"

        status = "ALERT" if (current_temp > 70.0 or vibration_hz > 90.0) else "NORMAL"
        breakdown_soon = 1 if (current_temp > 68.0 or vibration_hz > 90.0) else 0
